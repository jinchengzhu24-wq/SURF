...
PC_LEVEL_GENERATION_SYSTEM_PROMPT = (
    ...
    "from another water option. Walls must use cells whose canPlaceWall flag is "
    "true, and the player must use a cell whose canPlacePlayer flag is true. "
    "Prefer wall cells with higher wallImpactScore values and combine the walls "
    "into a barrier across direct box or player routes. The complete wall set "
    "must measurably affect the optimal solution: compared with removing only "
    "the generated walls, it must add at least four shortest-path movement "
    "steps or at least one minimum push. Decorative walls away from useful "
    "routes are invalid. "
