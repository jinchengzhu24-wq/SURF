...
class CoCreationPrototypeApiTests(unittest.TestCase):
    ...
    def test_frontend_and_static_assets_are_served(self):
        ...
        self.assertIn("cocreation-return-unity-20260814-19", index_response.text)
        self.assertIn("cocreation-return-unity-20260814-22", index_response.text)
        ...
        self.assertIn('|| !state.session.matchId', js_response.text)
        self.assertIn('|| ![1, 2].includes(Number(state.session.playerNumber));', js_response.text)
