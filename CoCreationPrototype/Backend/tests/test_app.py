...
class CoCreationPrototypeApiTests(unittest.TestCase):
    ...
    def test_frontend_and_static_assets_are_served(self):
        ...
        self.assertIn("draftPreviewGrid", index_response.text)
        ...
        self.assertIn("width: min(100%, 400px)", css_response.text)
        ...
        self.assertIn("selectedStageHasPendingIntentConflict", js_response.text)
        self.assertIn('t("intentKeepThis")', js_response.text)
        self.assertIn('selectedHypothesisId: action === "keep"', js_response.text)
        self.assertIn("intentConflictPlaceholder", js_response.text)
        self.assertIn(".intent-conflict-choice", css_response.text)
        self.assertIn('aria-pressed="false"', index_response.text)
        self.assertIn("--progress-panel-max-height", css_response.text)
        self.assertIn("overflow-y: auto", css_response.text)
        ...
        self.assertNotIn('const unityWindow = window.opener;', js_response.text)
        self.assertNotIn('window.close();', js_response.text)
        self.assertIn("function hasEmbeddedFormalHost()", js_response.text)
        self.assertIn("function restoreUnsavedMapDraft()", js_response.text)
        self.assertIn("function persistUnsavedMapDraft()", js_response.text)
        self.assertIn('tryOpenPlayInExistingUnity(payload.playUrl)', js_response.text)
        self.assertIn('type: "sokoban:cocreation-play-request"', js_response.text)
        self.assertIn('message.type !== "sokoban:cocreation-play-ack"', js_response.text)
        self.assertIn('message.type !== "sokoban:cocreation-play-return"', js_response.text)
        self.assertIn('if (!state.session.demoMode', js_response.text)
        self.assertIn('window.location.assign(payload.playUrl);', js_response.text)
        ...
        self.assertNotIn('id="demoButton"', index_response.text)
        self.assertNotIn('id="demoGenerationStatus"', index_response.text)
        self.assertIn('id="draftPreviewLoading"', index_response.text)
        self.assertIn('class="draft-preview-spinner"', index_response.text)
        self.assertIn('void createDemoSession();', js_response.text)
        self.assertIn('showLanding("demo-loading")', js_response.text)
        self.assertIn("draftPreviewLoading", js_response.text)
        ...
        self.assertIn("demoDraftPreviewTitle", js_response.text)
        ...
        self.assertLess(render_source.index("applyTranslations();"), render_source.index("setStatus(t(\"ready\"), \"ready\");"))
        ...
        self.assertIn("function isKnownCoCreationSource", webgl_template)
        self.assertNotIn("coCreationStandaloneWindow", webgl_template)
        self.assertNotIn("OPEN SEPARATE PAGE", webgl_template)
        ...
        self.assertIn('pendingError.code = "PENDING_MESSAGE";', js_response.text)
