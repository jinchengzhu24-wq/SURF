...
def _legacy_unclear_revision_intent(language):
    if language == "zh-CN":
        return (
...
def _unclear_revision_intent(language, latest_user=None):
    seed = (
        "\u6211\u6682\u65f6\u7406\u89e3\u5230\u4f60\u5e0c\u671b\u8c03\u6574\u5f53\u524d\u5173\u5361\uff0c\u4f46\u5177\u4f53\u8bbe\u8ba1\u91cd\u70b9\u8fd8\u9700\u8981\u7531\u4f60\u786e\u8ba4\u3002"
        if language == "zh-CN"
        else "For now, I understand that you want the current level adjusted, but the specific design priority still needs your confirmation."
    )
    return _detailed_intent_fallback(seed, latest_user, language)


def _unclear_revision_reply(language, latest_user=None):
    source = str(latest_user or "")
...
def _intent_hypothesis_detail_issue(value, language):
    ...
    if re.search(
        ...
        return "intentHypothesis combines incompatible directions"
    if re.search(
        r"(?:\u8fd9\u8bc1\u660e\u4f60|\u56e0\u4e3a.{0,100}\u6240\u4ee5\u4f60\u4e00\u5b9a|\b(?:this )?proves you\b|\bbecause\b.{0,120}\byou (?:definitely|certainly) (?:want|prefer)\b)",
        text,
        re.IGNORECASE,
    ):
...
def _detailed_intent_fallback(hypothesis, latest_user, language):
    ...
    core = next(
        (
            part.strip().rstrip(".!?")
            for part in re.split(r"(?<=[.!?])\s*", str(hypothesis or "").strip())
            if part.strip()
        ),
        "For now, I understand that you may have a preference about the current design effect",
    )
    if visual_only:
        return (
...
def _classify_revision_request(conversation, stage_context=None):
    ...
    if (stage_context or {}).get("vagueAestheticRevision"):
        ...
        # as a vague visual reaction. Do not let proposal-like wording in an
        # earlier assistant opening turn supply an inherited brief and upgrade
        # this request behind the user's back.
