...
- The 8010 LLM is an adaptive, conversational design peer rather than a fixed assessment form. Stage 1, `human_edit`, and restored Stages get one neutral grounded opening with two or three short paragraphs: select one or two concrete authored choices, offer at least one clearly subjective perspective or concern, and ask one open question about that choice. Do not use workflow greetings, inventory the map, ask for an overall player-experience category, or anchor the designer with an either-or choice. Later turns respond to the designer first, add a useful independent view when appropriate, and ask at most one question only when it moves the discussion forward. A Stage created by accepting an LLM proposal must not receive another first assessment: the original assistant turn that generated the accepted proposal becomes that new Stage's carried opening, and later dialogue continues from it. Later turns may clarify, offer a perspective, challenge a trade-off, reflect on play evidence, or offer a revision direction. Any inferred intention must remain tentative and correctable and must never populate the designer's final intention.
...
- Assistant guidance uses structured, accessible UI cues inside the same message bubble: manual editor hints are green, questions are blue, revision directions/proposals are purple, tentative intention hypotheses are orange, and warnings or evidence-grounded potential risks are red. Each cue is bold and carries a bilingual text label. New `manual_edit` and `warning` cues live in the optional `guidance.uiCues` array; historical `tradeoff` cues remain compatible and render as warnings, while historical turns are never keyword-classified.
...
- Each 8010 business request has a 60-second wall-clock hard limit. Ordinary chat and Stage openings prefer `deepseek-v4-flash`; explicit complete-map proposals prefer `deepseek-v4-pro`. The preferred model gets at most 40 seconds, then the other model may use only the remaining total time. DeepSeek thinking output is explicitly disabled so reasoning tokens cannot consume the whole visible JSON budget. If a model duplicates a question in `assistantMessage`, the backend removes only question-bearing paragraphs and only when a structured `followUpQuestion` exists; it must still reject unstructured questions. Both attempts share one request and idempotency key, and only the successful result is persisted. The browser displays elapsed time and applies a 65-second safety abort. Timeout, refresh, or connection recovery must reuse the original key so one logical request has at most one user turn and one assistant turn. Reusing a key with different content or a different Stage is an idempotency conflict.
...
- The independent 8010 FastAPI service, persistent SQLite/WAL schema, and three-column bilingual frontend were deployed on 2026-08-11. The 60-second wall-clock hard limit, 40-second preferred-model window, Flash/Pro fallback, message-idempotency recovery, elapsed-time UI, larger map-panel text, editor-aware guidance, verified human-edit acknowledgement, Stage-scoped conversation, accepted-proposal Stage opening, global bold typography, and five-color guidance cues were deployed on 2026-08-12 with frontend cache keys `cocreation-pixel-20260812-5` and `cocreation-guided-20260812-9`. Thinking-disabled structured generation and safe duplicate-question normalization were deployed later that day to fix empty/invalid first Stage assessments. The deployed workbench uses adaptive prompt `cocreation-v9-conversational-guidance`, keeps complete solver move traces out of LLM context, and labels blue questions as `LET'S DISCUSS / 一起聊聊`; an old first-assessment turn already recorded for an accepted-proposal Stage remains in SQLite for research integrity but is excluded from that Stage's UI and future LLM context. Existing databases gain nullable `conversation_turns.guidance_json` without rewriting old turns. Its systemd service reads a separate protected `.env`, reports `tokenSecretConfigured: true`, and stores data at `/root/SURF/CoCreationPrototype/Backend/data/cocreation.sqlite3`.
...
- Current verification on 2026-08-12: the unchanged main backend remains at its previously verified 144-test baseline; the independent `CoCreationPrototype/Backend` suite passes 58 tests locally and on the server, and `node --check CoCreationPrototype/Frontend/app.js` passes locally. Live non-persistent English and Chinese Stage-opening probes confirmed no workflow greeting, no solver trace, and a three-part rendered opening; the first probe also revealed model-generated either-or and yes/no anchoring, so the final backend deterministically normalizes recoverable concrete questions and rejects unsafe ones. A final Chinese Flash probe completed in about 6.2 seconds with an open, non-anchored question. None of the probes wrote a session turn. The deployed service reports healthy/ready with zero automatic restarts. The latest Unity verification remains `dotnet build Assembly-CSharp.csproj -v:minimal` with 0 errors and a WebGL build with 0 errors and 0 warnings; neither Unity nor 8000 was rebuilt for these 8010-only updates. A future `--no-restore` build requires the generated `Temp/obj/.../project.assets.json` to exist first.
