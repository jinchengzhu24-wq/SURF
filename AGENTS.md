...
- Each 8010 business request has a 60-second wall-clock hard limit. Ordinary chat and Stage openings prefer `deepseek-v4-flash`; explicit complete-map proposals prefer `deepseek-v4-pro`. The preferred model gets at most 40 seconds, then the other model may use only the remaining total time. Both attempts share one request and idempotency key, and only the successful result is persisted. The browser displays elapsed time and applies a 65-second safety abort. Timeout, refresh, or connection recovery must reuse the original key so one logical request has at most one user turn and one assistant turn. Reusing a key with different content or a different Stage is an idempotency conflict.
...
- Current verification on 2026-08-12: the unchanged main backend remains at its previously verified 144-test baseline; the independent `CoCreationPrototype/Backend` suite passes 46 tests locally and on the server, `node --check CoCreationPrototype/Frontend/app.js` passes locally, and a live non-persistent Flash chat probe completed in one attempt with about 12 seconds of server latency. A live map-proposal probe confirmed that Pro was cancelled at 40 seconds and the complete request stopped within 60 seconds; its Flash fallback returned invalid structured output and was safely rejected without persistence, demonstrating that Pro is not a latency guarantee. The latest Unity verification remains `dotnet build Assembly-CSharp.csproj -v:minimal` with 0 errors and a WebGL build with 0 errors and 0 warnings; neither Unity nor 8000 was rebuilt for these 8010-only updates. A future `--no-restore` build requires the generated `Temp/obj/.../project.assets.json` to exist first.
