...
- The independent 8010 FastAPI service, persistent SQLite/WAL schema, and three-column bilingual frontend were deployed on 2026-08-11. The 60-second single-call timeout, composer-local retry status, and message-idempotency recovery update was deployed on 2026-08-12 with frontend cache keys `cocreation-pixel-20260812-1` and `cocreation-guided-20260812-1`. The deployed workbench uses the Pixel-adventure theme and adaptive guidance prompt `cocreation-v2-guided`; existing databases gain nullable `conversation_turns.guidance_json` without rewriting old turns. Its systemd service reads a separate protected `.env`, reports `tokenSecretConfigured: true`, and stores data at `/root/SURF/CoCreationPrototype/Backend/data/cocreation.sqlite3`.
...
- Current local verification on 2026-08-12: the unchanged main backend remains at its previously verified 144-test baseline; the independent `CoCreationPrototype/Backend` suite passes 35 tests, and `node --check CoCreationPrototype/Frontend/app.js` passes. The latest Unity verification remains `dotnet build Assembly-CSharp.csproj -v:minimal` with 0 errors and a WebGL build with 0 errors and 0 warnings; neither Unity nor 8000 was rebuilt for the 8010-only reliability update. A future `--no-restore` build requires the generated `Temp/obj/.../project.assets.json` to exist first.
