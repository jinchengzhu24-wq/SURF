...
- The independent 8010 FastAPI service, persistent SQLite/WAL schema, and three-column bilingual frontend were deployed on 2026-08-11. The 60-second wall-clock hard limit, 40-second preferred-model window, Flash/Pro fallback, message-idempotency recovery, elapsed-time UI, larger map-panel text, editor-aware guidance, verified human-edit acknowledgement, Stage-scoped conversation, accepted-proposal Stage opening, global bold typography, and five-color guidance cues were deployed on 2026-08-12 with frontend cache keys `cocreation-pixel-20260812-5` and `cocreation-guided-20260812-8`. Thinking-disabled structured generation and safe duplicate-question normalization were deployed later that day to fix empty/invalid first Stage assessments. The deployed workbench uses adaptive prompt `cocreation-v8-nonthinking-normalized`; an old first-assessment turn already recorded for an accepted-proposal Stage remains in SQLite for research integrity but is excluded from that Stage's UI and future LLM context. Existing databases gain nullable `conversation_turns.guidance_json` without rewriting old turns. Its systemd service reads a separate protected `.env`, reports `tokenSecretConfigured: true`, and stores data at `/root/SURF/CoCreationPrototype/Backend/data/cocreation.sqlite3`.
...
- Current verification on 2026-08-12: the unchanged main backend remains at its previously verified 144-test baseline; the independent `CoCreationPrototype/Backend` suite passes 49 tests locally and on the server, and `node --check CoCreationPrototype/Frontend/app.js` passes locally. After disabling thinking output, a live non-persistent Stage-opening probe completed through the Pro fallback in about 15.7 seconds with a complete assessment and structured question; a separate Flash structure probe also validated successfully. Neither probe wrote a session turn. The latest Unity verification remains `dotnet build Assembly-CSharp.csproj -v:minimal` with 0 errors and a WebGL build with 0 errors and 0 warnings; neither Unity nor 8000 was rebuilt for these 8010-only updates. A future `--no-restore` build requires the generated `Temp/obj/.../project.assets.json` to exist first.
