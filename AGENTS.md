...
- A `proposalOffer` may carry a hidden `executionBrief` for exact execution: `effect`, `anchors`, `focus`, `requiredTransitions`, `allowedOperators`, `preserve`, and `playObjective`. Explicit coordinates and `from → to` transitions are hard constraints; validate them against current `tileAt` facts and require clarification on conflicts. A precise one-cell structural edit may use one changed cell; entity relocation still requires paired cells. If modifier-agent candidates are invalid, production may use the bounded deterministic `search_revision_plan()` fallback, but it must still replay the same contract and solver checks.
- Within the current Stage, only the latest assistant turn with a valid `proposalOffer` may receive `execute_revision`, `challenge_revision`, or `alternative_revision`. Older purple cards remain visible but read-only in the frontend, and the backend must reject stale `sourceTurnId` values with `INVALID_CARD_SOURCE`. Ordinary assistant turns without `proposalOffer` do not make the latest proposal stale; accepted map-proposal controls are separate.
