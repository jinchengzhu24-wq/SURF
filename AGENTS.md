...
- The 8010 LLM is an adaptive co-creation partner rather than a fixed assessment form. Each saved Stage gets one neutral grounded opening; later turns may clarify, offer a perspective, challenge a trade-off, reflect on play evidence, or offer a revision direction. Any inferred intention must remain tentative and correctable and must never populate the designer's final intention.
...
- Conversation is Stage-scoped without deleting research records. Every turn retains its `version_id`; the frontend shows only turns for the selected Stage, historical Stage chat is read-only, and `build_llm_context` supplies only turns attached to the Stage being assessed or discussed. Composer drafts and pending message recovery belong only to the current Stage.
...
- The independent 8010 FastAPI service, persistent SQLite/WAL schema, and three-column bilingual frontend were deployed on 2026-08-11. The 60-second single-call timeout, message-idempotency recovery, larger map-panel text, editor-aware guidance, verified human-edit acknowledgement, and Stage-scoped conversation updates were deployed on 2026-08-12 with frontend cache keys `cocreation-pixel-20260812-2` and `cocreation-guided-20260812-3`. The deployed workbench uses adaptive prompt `cocreation-v4-stage-scoped`; existing databases gain nullable `conversation_turns.guidance_json` without rewriting old turns. Its systemd service reads a separate protected `.env`, reports `tokenSecretConfigured: true`, and stores data at `/root/SURF/CoCreationPrototype/Backend/data/cocreation.sqlite3`.
