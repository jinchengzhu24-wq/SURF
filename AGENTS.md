...
- The 8010 frontend follows the active 8000 Pixel-adventure dashboard theme: grass-grid background, wooden topbar, stone panels, blue/purple title plates, hard pixel shadows, and monospace display text. It keeps its own responsive Stage/chat/map layout and independent CSS.
...
- The independent 8010 FastAPI service, persistent SQLite/WAL schema, and three-column bilingual frontend were deployed on 2026-08-11. The 60-second single-call timeout, message-idempotency recovery, larger map-panel text, editor-aware guidance, verified human-edit acknowledgement, Stage-scoped conversation, and accepted-proposal Stage opening were deployed on 2026-08-12 with frontend cache keys `cocreation-pixel-20260812-2` and `cocreation-guided-20260812-5`. The deployed workbench uses adaptive prompt `cocreation-v5-accepted-opening`; an old first-assessment turn already recorded for an accepted-proposal Stage remains in SQLite for research integrity but is excluded from that Stage's UI and future LLM context. Existing databases gain nullable `conversation_turns.guidance_json` without rewriting old turns. Its systemd service reads a separate protected `.env`, reports `tokenSecretConfigured: true`, and stores data at `/root/SURF/CoCreationPrototype/Backend/data/cocreation.sqlite3`.
...
- Current local verification on 2026-08-12: the unchanged main backend remains at its previously verified 144-test baseline; the independent `CoCreationPrototype/Backend` suite passes 39 tests, and `node --check CoCreationPrototype/Frontend/app.js` passes. The latest Unity verification remains `dotnet build Assembly-CSharp.csproj -v:minimal` with 0 errors and a WebGL build with 0 errors and 0 warnings; neither Unity nor 8000 was rebuilt for these 8010-only updates. A future `--no-restore` build requires the generated `Temp/obj/.../project.assets.json` to exist first.
