...
- Assistant guidance uses structured, accessible UI cues inside the same message bubble: manual editor hints are green, questions are blue, revision directions/proposals are purple, and trade-off warnings are orange. Each cue is bold and carries a bilingual text label. New `manual_edit` and `tradeoff` cues live in the optional `guidance.uiCues` array; historical turns are never keyword-classified.
...
- Each 8010 LLM request uses one upstream call with a 60-second timeout. The chat composer persists its content, Stage id, and idempotency key before sending; timeout, refresh, or connection recovery must reuse that key so one logical request has at most one user turn and one assistant turn. Reusing a key with different content or a different Stage is an idempotency conflict.
...
- The independent 8010 FastAPI service, persistent SQLite/WAL schema, and three-column bilingual frontend were deployed on 2026-08-11. The 60-second single-call timeout, message-idempotency recovery, larger map-panel text, editor-aware guidance, verified human-edit acknowledgement, Stage-scoped conversation, accepted-proposal Stage opening, global bold typography, and four-color guidance cues were deployed on 2026-08-12 with frontend cache keys `cocreation-pixel-20260812-4` and `cocreation-guided-20260812-7`. The deployed workbench uses adaptive prompt `cocreation-v6-ui-cues`; an old first-assessment turn already recorded for an accepted-proposal Stage remains in SQLite for research integrity but is excluded from that Stage's UI and future LLM context. Existing databases gain nullable `conversation_turns.guidance_json` without rewriting old turns. Its systemd service reads a separate protected `.env`, reports `tokenSecretConfigured: true`, and stores data at `/root/SURF/CoCreationPrototype/Backend/data/cocreation.sqlite3`.
...
- Current local verification on 2026-08-12: the unchanged main backend remains at its previously verified 144-test baseline; the independent `CoCreationPrototype/Backend` suite passes 43 tests, and `node --check CoCreationPrototype/Frontend/app.js` passes. The latest Unity verification remains `dotnet build Assembly-CSharp.csproj -v:minimal` with 0 errors and a WebGL build with 0 errors and 0 warnings; neither Unity nor 8000 was rebuilt for these 8010-only updates. A future `--no-restore` build requires the generated `Temp/obj/.../project.assets.json` to exist first.
