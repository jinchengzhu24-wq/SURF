...
8010 共创服务、8000 中立匹配后端和包含 Stage Play 的 WebGL 已部署到 `http://111.231.136.4:8010/` 与 `http://111.231.136.4:8000/game/`。2026-08-13 的 8010 对话版本使用提示词 `cocreation-v22-distilled-guidance-reliable-proposals`，以自然朋友式语气、稍充分的观察与推理、仅首次 Stage 1 开场无问题、经过提炼或展开的蓝色讨论卡与紫色建议卡、自然修改授权、强证据风险卡和“单关卡、多版本”语义为准；8010 前端脚本缓存键为 `cocreation-distilled-guidance-20260813-14`，WebGL 仍为此前使用 Unity 2022.3.62f2c1 构建的版本。后续修改仍须按本文验证流程重新构建和部署，不能仅凭本地源码判断线上版本。
