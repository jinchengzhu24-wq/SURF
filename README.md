...
8010 共创服务、8000 中立匹配后端和包含 Stage Play 的 WebGL 已部署到 `http://111.231.136.4:8010/` 与 `http://111.231.136.4:8000/game/`。2026-08-13 的 8010 对话版本使用提示词 `cocreation-v20-single-level-dialogue`，以自然朋友式语气、仅首次 Stage 1 开场无问题、关键节点具体追问或独立见解、强证据风险卡和“单关卡、多版本”语义为准；8010 前端沿用现有缓存键，WebGL 仍为此前使用 Unity 2022.3.62f2c1 构建的版本。后续修改仍须按本文验证流程重新构建和部署，不能仅凭本地源码判断线上版本。
...
LLM 使用理性、亲切、以第一人称为主的朋友式共创策略，并明确避免机械复述、固定开头、客服话术及“回应—评价—提问”模板。只有整段会话最开始的 Stage 1 首条开场不提问：它只挑一两个具体选择表达个人观点，并自然提示玩家可以分享感受、试玩或在右侧做局部编辑。从下一条对话起，即使仍停留在 Stage 1，只要评价不清、出现真实取舍、方向开始可执行或获得新试玩证据，助手就可以用具体追问或自己的独立见解继续引导；蓝色 `LET'S DISCUSS / 一起聊聊` 卡可以承载这两种内容。玩家已表达的偏好不会被再次要求确认，但可以引出更深一层的路线、推动顺序或可读性判断；连续蓝卡必须推进不同判断。整个共创会话始终只有一关，所有 Stage 都只是这同一关的保存版本；提示词与保存前规范化会拦截“第二关”“下一关”“后面关卡”等错误进度表述。接受 LLM 地图提案所创建的 Stage 不再另做 first assessment，而把生成该提案的原 assistant 回复作为新 Stage 的承接开场。意图推测必须可纠正、贴合当前讨论，并以自然多样的第一/第二人称表达；第三人称报告口吻会在保存前规范化。助手气泡继续使用绿色手工编辑、蓝色讨论、紫色修改建议、橙色暂定意图和红色警告卡。红色警告仅用于明确试玩异常、具体地图元素之间可解释的机械风险、具有直接后果的版本变化或与玩家方向的明确冲突；普通不确定性和审美、策略取舍保留在正文。API、历史记录和完整地图提案的严格验证流程保持不变。
