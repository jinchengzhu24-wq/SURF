...
普通聊天会对明显的求助请求做确定性卡片路由：已有具体体验目标或修改方向、并请求建议、方案或修改方法时，优先出现 `REVISION / 修改建议` 卡；明确要求“帮我改”但没有具体方向时，先在正文中说明当前理解并显示可纠正的 `TENTATIVE INTENT / 暂定意图`，不直接形成修改方案；普通事实问题和开放式求思路留在正文。输入框右侧的“方案 / Proposal”是一次性模式开关，本身不发送；开启后再用 Enter 或“发送”才将用户原文以 `requestProposal=true` 提交并强制进入同一方案发现流程，成功后自动恢复普通聊天。修改建议卡只是概念性方向，点击“请助手生成这个方案”后才进入授权提案流程，不会自动修改地图。
