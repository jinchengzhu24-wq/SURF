...
普通聊天会对明显的求助请求做确定性卡片路由：已有具体体验目标或修改方向、并请求建议、方案或修改方法时，优先出现 `REVISION / 修改建议` 卡；没有明确方向、表现出迷茫并请求思路时，优先出现 `LET'S DISCUSS / 一起聊聊` 卡。两者同时满足时修改建议优先。修改建议卡只是概念性方向，点击“请助手具体生成这个方案”后才进入授权提案流程，不会自动修改地图；“帮我改”但没有方向仍沿用手动编辑引导。
